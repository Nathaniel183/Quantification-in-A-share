...
class StgPattern(Strategy):
    ...
    def __init__(self, cash: float, datas: pd.DataFrame, save:bool=True):
        ...
        # 设置策略名
        self.strategy_name = "NSQ策略"

        self.total_invest = self.init_cash
    ...
    def next(self):
        ...
        # 计算购买数量
        counts = []
        c = self.count_compute('nsq', 1, 1)
        counts.append({'code': 'nsq', 'count': c})
    ...
    def custom_finally(self):
        # 自定义结束处理，程序结束后执行
        print(f'总投入：{self.total_invest}，对初始资金比例：{self.total_invest/self.init_cash*100:.2f}%')


if __name__ == '__main__':
    # 策略初始化
    strategy = StgPattern(cash=10000000,
                          datas=get_nsq_m('20161201', '20260201'),
                          save=False)
